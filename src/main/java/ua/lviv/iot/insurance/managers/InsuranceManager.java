...
public class InsuranceManager implements SortOfMethodsInsurance {
    ...
    @Override
    public final List<Insurance> sortingBasedOnRiskReduction(
            final boolean descending) {
        Comparator<Insurance> comparator = (Insurance obj1,
                Insurance obj2) -> Integer.compare(obj2.getNumberOfDays(),
    ...
    @Override
    public final List<Insurance> sortingAlphabeticallyInAscendingOrder(
            final boolean descending) {
        Comparator<Insurance> comparator = (Insurance obj1,
                Insurance obj2) -> obj1.getName().compareTo(obj2.getName());
        insurance.sort(comparator);
        return insurance;
    }

    @Override
    public final List<Insurance> sortingAlphabeticallyInDescendingOrder(
            final boolean descending) {
        Comparator<Insurance> comparator = (Insurance obj1,
                Insurance obj2) -> obj2.getName().compareTo(obj1.getName());
        insurance.sort(comparator);
        return insurance;
    }
    ...
    public final List<Insurance> findBy(final InsuranceType type) {
        return insurance.stream().filter(ins -> ins.getInsuranceType() == type)
